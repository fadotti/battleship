...
- Gameboards have 6 properties: `grid` is an array of length 100 that represents the 10x10 board. The five remaining properties are each of the player's ships: a destroyer, a submarine, a cruiser, a battleship, and a carrier. Gameboards have the following methods that are used to command the flow of the game:
  - `#isHorizontalPlacementAllowed` returns `true` if a given ship can be placed horizontally on a given square, and `false` otherwise.
  - `#isVerticalPlacementAllowed` returns `true` if a given ship can be placed vertically on a given square, and `false` otherwise.
  - `placeShip` places a ship on the gameboard if the action is allowed.
  - `receiveAttack` receives an attack at the given square: if a ship is on that square, it will take a hit.
...
This application consists of a single URL, and the production build has been created with webpack. The HTML is manipulated exclusively via the DOM. `.../src/index.js` is the entry file, and all other scripts are connected to it. There are three main screens: the landing page, the board setup page, and the arena. Each of these have their own script that contains a function that renders the associated screen when called. `.../src/event-handlers.js` contains functions that add event listeners to each of the screens, with the exception of the arena's event listeners which are included directly in `.../src/arena.js` and which control the flow of the game.
